...
AABB SetAABB3(DirectX::SimpleMath::Vector3 centerposition, float width, int height, int depth)
{
	AABB aabb{};
	...
	aabb.min.x = centerposition.x - width;
	aabb.min.y = centerposition.y ;
	aabb.min.z = centerposition.z - depth;

	aabb.max.x = centerposition.x + width;
	aabb.max.y = centerposition.y + height * 2.0f;
	aabb.max.z = centerposition.z + depth;

	return aabb;
}

AABB SetAABB2(DirectX::SimpleMath::Vector3 centerposition, float x_pos, float y_pos, float width, int height)
{
	AABB aabb{};
	...
	aabb.min.x = centerposition.x - x_pos - (width / 2.0f);
	aabb.min.y = centerposition.y - y_pos - (height / 2.0f);
	aabb.min.z = 0.0f;

	aabb.max.x = centerposition.x + x_pos + (width / 2.0f);
	aabb.max.y = centerposition.y + y_pos + (height / 2.0f);
	aabb.max.z = 0.0f;
